...
module EmailHelpers
  def current_email_address
    ...
    last_email_address || 'user1@mail.com'
  end
end
...
Then /^(?:я|"([^"]*?)") должен получить (1|0|[2-9]+) новы(?:х|й) емейл(?:а|ов)?$/ do |address, amount|
  amount = num_to_english(amount)
  expect(unread_emails_for(address).size).to eq(parse_email_count(amount))
end

Then /^(?:(?:у меня)|(?:в ящике "([^"]*?)")) всего долж(?:но|ен) быть (1|0|[2-9]+) емейл(?:а|ов)?$/ do |address, amount|
  amount = num_to_english(amount)
